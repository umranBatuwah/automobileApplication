...
public class CarGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all cars")
                    .get("/services/automobileapplication/api/cars")
                ...
                .exec(
                    http("Create new car")
                        .post("/services/automobileapplication/api/cars")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"carName\": \"SAMPLE_TEXT\"" + ", \"numberOfCar\": 0" + ", \"carMadeBy\": \"SAMPLE_TEXT\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                ...
                .repeat(5)
                .on(
                    exec(http("Get created car").get("/services/automobileapplication${new_car_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created car").delete("/services/automobileapplication${new_car_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );
